...
describe("QRGenerator", () => {
  ...
  it("shows an error when the value is empty", () => {
    ...
    expect(
      screen.getByText("No se encontro contenido para generar el QR."),
    ).toBeInTheDocument();
  });
  ...
  it("generates a QR image and passes normalized colors to qrcode", async () => {
    ...
    expect(await screen.findByAltText("Código QR GLUD")).toHaveAttribute(
      "src",
  ...
  it("muestra mensaje de error si qrcode falla", async () => {
    ...
    expect(
      await screen.findByText("No se pudo generar el QR."),
    ).toBeInTheDocument();
  });
